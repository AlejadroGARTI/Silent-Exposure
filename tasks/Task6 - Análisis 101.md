...
Se emplearon herramientas de análisis estático y comandos de Linux para obtener información importante de la máquina y de su funcionamiento.
...
Mediante `apt --version`, vemos que el paquete tiene la versión: `apt 1.2.32ubuntu0.1 (amd64)`, y esto es crucial ya que saber la versión exacta de apt permite buscar vulnerabilidades conocidas o CVEs por sus siglas en inglés, que afecten a esa versión específica, además la versión 1.2.32ubuntu0.1 corresponde a una versión antigua (Ubuntu 16.04), lo que sugiere que el sistema no está actualizado y podría ser vulnerable a exploits públicos, por lo que se pueden buscar exploits listos para usar que permitan escalar privilegios o comprometer el gestor de paquetes.
